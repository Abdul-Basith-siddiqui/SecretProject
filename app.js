...
app.use(
  session({
    //initializing the session and setting some defaults , saves the user  login session
...
const userSchema = new mongoose.Schema({
  //authentication -- this sehema is no longer javascript obj but a mongoose sehema obj , created from mongoose class what we require
  ...
  new GoogleStrategy(
    ...
    function (accessToken, refreshToken, profile, cb) {
      ...
      User.findOrCreate(
        ...
        function (err, user) {
          //find inthe database then send get request or create new user inthe database with google provided id then make a get request
          return cb(err, user);
        }
      );
    }
...
app.get(
  "/auth/google/secretss", // this get request is send by google and then we can authenticate locally
...
app.get("/submit", function (req, res) {
  if (req.isAuthenticated()) {
    //from passportLocalMongoose  For any request you can check if a user is authenticated or not using this method.
    res.render("submit");
  } else {
    res.render("login");
  }
...
app.get("/logout", (req, res) => {
  req.logout((err) => {
    //from passportLocalMongoose   Invoking logout() will remove the req.user property and clear the login session (if any).
    ...
    function (err, user) {
      ...
      if (err) {
        ...
        passport.authenticate("local")(req, res, function () {
          //this method comes from passport - successfully setup a cookies and save the current logged session
...
app.post("/login", function (req, res) {
  ...
  req.login(user, (err) => {
    //this method comes from passport, creates a session
    if (err) {
      console.log(err);
    } else {
      passport.authenticate("local")(req, res, function () {
